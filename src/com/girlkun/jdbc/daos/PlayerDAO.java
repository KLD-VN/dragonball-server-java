...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            String linhthuegg = dataArray.toJSONString();
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, data_linhthu_egg) "
                    ...
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, linhthuegg);
        ...
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                if (player.linhthuegg != null) {
                    dataArray.add(player.linhthuegg.lastTimeCreate);
                    dataArray.add(player.linhthuegg.timeDone);
                }
                String linhthuegg = dataArray.toJSONString();
                dataArray.clear();
                //data bùa
                ...
                String query = " update player set head = ?, have_tennis_space_ship = ?,"
                        ...
                        + "data_black_ball = ?, data_side_task = ?, data_charm = ?, skills = ?, skills_shortcut = ?, pointPvp=?, data_linhthu_egg=?, maxPvpClone=?, Pity=? where id = ?";
    ...
    public static boolean subVnd(Player player, int num) {
        PreparedStatement ps = null;
        try (Connection con = GirlkunDB.getConnection();) {
            ps = con.prepareStatement("update account set balance = (balance - ?), active = ?, is_gift_box = ? where id = ?");
            ps.setInt(1, num);
            ps.setInt(2, player.getSession().actived ? 1 : 0);
            ps.setInt(3, player.getSession().is_gift_box ? 1 : 0);
            ps.setInt(4, player.getSession().userId);
            ps.executeUpdate();
            player.getSession().balance -= num;
        } catch (Exception e) {
            Logger.logException(PlayerDAO.class, e, "Lỗi update thỏi vàng " + player.name);
            return false;
        }
