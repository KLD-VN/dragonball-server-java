    ...
    private void udPlayer() {
        for (int i = this.notBosses.size() - 1; i >= 0; i--) {
            Player pl = this.notBosses.get(i);
             if (!pl.isPet && !pl.isNewPet && !pl.isPetEvent) {
    ...
    public void addPlayer(Player player) {
        if (player != null) {
            ...
            if (!player.isBoss && !this.notBosses.contains(player)) {
             ...
             if (!player.isBoss && !player.isNewPet && !player.isPetEvent && !player.isPet && !this.players.contains(player)) {
                this.players.add(player);
            }
            if (player.isBoss) {
                this.bosses.add(player);
            }
           if (player.isPet || player.isNewPet  || player.isPetEvent) {
                            ...
                            default:
                                switch (item.template.id) {
                                    case 73:
                                        ...
                                        break;
                                    case 74:
                                        msg.writer().writeUTF("Mày mới vừa ăn " + item.template.name);
                                        break;
                                    case 78:
                                    ...
                                    default:
                                        if (item.template.type >= 0 && item.template.type < 5) {
                                            ...
                                            msg.writer().writeUTF("Mày mới nhặt được " + item.template.name);
                                        }
...
//            Logger.logException(MapService.class, e);
      ...
      if (!plInfo.isBoss && !plInfo.isPet && !plInfo.isNewPet && !plInfo.isPetEvent) {
