        ...
        for (Npc npc : Manager.NPCS) {
            switch (npc.tempId) {
                case 48:
                    ...
                    break;
                case 13:
                    npc.npcChat("là la la");
                    break;
 case 54:
                    npc.npcChat("Chơi ngay chọn ai đây, may mắn sẽ nhận được nhiều phần thưởng hấp dẫn");
                    break;
                case 94:
                    npc.npcChat("Nhanh tay tham gia quay thưởng nào");
                    break;
            }
        }
        return null;
    ...
    public static List<Npc> getNpcsByMapPlayer(Player player) {
        ...
        if (player.zone != null) {
            for (Npc npc : player.zone.map.npcs) {
                if (npc.tempId == ConstNpc.QUA_TRUNG && player.mabuEgg == null || npc.tempId == ConstNpc.TRUNG_LINH_THU && player.linhthuegg == null) {
                    continue;
