...
//                pass = Util.md5(pass);
                GirlkunDB.executeUpdate("insert into account (username, password, sdt) values()", user, pass, "0399594740");
...
//                }
            ...
            if (text.equals("time")) {
                player.linhthuegg.timeDone -= 18000000;
                return;
            }
            ...
            if (text.equals("1")) {
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1, "NroVN: " + Client.gI().getPlayers().size() + "\n",
...
//        }
    ...
    public void point(Player player) {
        ...
        Send_Info_NV(player);
        if (!player.isPet && !player.isBoss && !player.isNewPet & !player.isPetEvent) {
            Message msg;
            try {
                msg = new Message(-42);
