            ...
            switch (player.zone.map.mapId) {
                case 6:
                    if (player.petevent != null) {
                        try {
                            Service.getInstance().chat(player.petevent, "Bần tăng cám ơn thí chủ đã giúp ta thỉnh kinh, xin hãy nhận lấy món quà này");

                            player.inventory.coupon += 10;
                            Service.getInstance().sendThongBao(player, "bạn vừa nhận được 10 điểm sự kiện. Điểm hiện tại: " + player.inventory.coupon);
                            Thread.sleep(3000);
                            player.petevent.dispose();
                            player.petevent = null;
