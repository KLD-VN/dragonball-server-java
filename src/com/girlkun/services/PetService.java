...
import com.girlkun.models.player.PetEvent;
...
public class PetService {
    ...
    public void createNgoKhongPet(Player player, byte... limitPower) {
        new Thread(() -> {
            try {
                ...
                Thread.sleep(1000);
                Service.getInstance().chatJustForMe(player, player.pet, "HAHAHA khá lắm, được rồi giờ ta sẽ là đệ tử của ngươi.");
            } catch (Exception e) {
            }
        }).start();
    }
    ...
    //wk
    public void changeNgoKhong(Player player, int gender) {
        byte limitPower = player.pet.nPoint.limitPower;
        if (player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
            player.pet.unFusion();
        }
    ...
    public void changeNgoKhong(Player player) {
        byte limitPower = player.pet.nPoint.limitPower;
        if (player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
            player.pet.unFusion();
        }
    ...
    private int[] getDataPetNgoKhong() {
        int[] hpmp = {1700, 1800, 1900, 2000, 2100, 2200};
        int[] petData = new int[5];
        petData[0] = Util.nextInt(40, 105) * 25; //hp
        petData[1] = Util.nextInt(40, 105) * 25; //mp
        petData[2] = Util.nextInt(50, 120); //dame
        petData[3] = Util.nextInt(9, 10); //def
        petData[4] = Util.nextInt(0, 5); //crit
        return petData;
    }
    private void createNewPet(Player player, boolean isMabu, boolean isNgoKhong, byte... gender) {
        int[] data = isMabu ? getDataPetMabu() : (isNgoKhong ? getDataPetNgoKhong() : getDataPetNormal());
        Pet pet = new Pet(player);
        pet.name = "$" + (isMabu ? "Mabư" : (isNgoKhong ? "NgộKhông" : "Đệ tử"));
        pet.gender = (gender != null && gender.length != 0) ? gender[0] : (byte) Util.nextInt(0, 2);
        pet.id = -player.id;
        pet.nPoint.power = isMabu ? 1500000 : (isNgoKhong ? 1000000 : 2000);
        pet.typePet = (byte) (isMabu ? 1 : (isNgoKhong ? 2 : 0));
    ...
    public static void Pet3(Player pl, int h, int b, int l) {  //npc su kien
        if (pl.petevent != null) {
            pl.petevent.dispose();
        }
        pl.petevent = new PetEvent(pl, (short) h, (short) b, (short) l);
        pl.petevent.name = "Đường tăng";
        pl.petevent.gender = pl.gender;
        pl.petevent.nPoint.tiemNang = 1;
        pl.petevent.nPoint.power = 1;
        pl.petevent.nPoint.limitPower = 1;
        pl.petevent.nPoint.hpg = 500000;
        pl.petevent.nPoint.mpg = 1;
        pl.petevent.nPoint.hp = 500000;
        pl.petevent.nPoint.mp = 1;
        pl.petevent.nPoint.dameg = 1;
        pl.petevent.nPoint.defg = 1;
        pl.petevent.nPoint.critg = 1;
        pl.petevent.nPoint.stamina = 1;
        pl.petevent.nPoint.setBasePoint();
        pl.petevent.nPoint.setFullHpMp();
        pl.petevent.typePk = 5;
    }
    //--------------------------------------------------------------------------
}
