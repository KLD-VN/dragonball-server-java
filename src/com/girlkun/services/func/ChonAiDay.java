...
//random ma giao dich va luu lai

    private static Random rand = new Random();
    private static int previousRandomNumber;

    public static int nextInt(int from, int to) {
        int randomNumber = from + rand.nextInt(to - from + 1);
        previousRandomNumber = randomNumber;
        return randomNumber;
    }

    public static int getPreviousRandomNumber() {
        ...
        while (true) {
            try {
                if (((ChonAiDay.gI().lastTimeEnd - System.currentTimeMillis()) / 1000) <= 0) {
                    ChonAiDay.nextInt(0000000, 9999999);
                    ...
                    if (!ChonAiDay.gI().PlayersVIP.isEmpty()) {
                        ...
                        if (listN.size() > 5) {
                            Player pl = listN.get(Util.nextInt(0, 5));
                            int magd = Util.nextInt(00000, 99999);
                            if (pl != null) {
                                ...
                                Service.getInstance().sendThongBao(pl, "Chúc mừng bạn đã dành chiến thắng và nhận được " + goldC + " thỏi vàng");
                                //   ServerNotify.gI().notify(pl.name + " đã chiến thắng Chọn ai đây giải VIP và nhận được " + goldC + " thỏi vàng" + " Mã giao dịch: " + magd);
                                Item it = ItemService.gI().createNewItem((short) 457, goldC);
                                InventoryServiceNew.gI().addItemBag(pl, it);
                                InventoryServiceNew.gI().sendItemBags(pl);
                            }
    ...
    private void sendNotifyShenronAppear(Player pl) {
        ...
        try {
            msg = new Message(-25);
            msg.writer().writeUTF(pl.name + "  đã chiến thắng Chọn ai đây giải VIP ");
            msg.cleanup();
        } catch (Exception e) {
        }
    }
